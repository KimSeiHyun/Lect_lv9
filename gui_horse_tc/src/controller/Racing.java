...
public class Racing extends MyUtil implements Runnable{
	...
	public Racing() {
		setLayout(null);
		setBounds(0,0,Race.WIDTH , Race.HEIGTH);
	...
	private void setTimer() {
		this.timer.setBounds(135,50,100,50);
		add(this.timer);
	}
	...
	protected void paintComponent(Graphics g) {
		...
		for(int i=0; i<SIZE; i++) {
			...
			if(h.getState() == h.GOAL) {
				...
				g.drawString(h.getRecord(), Race.WIDTH-200, h.getY()+h.getH()/2);
				g.setFont(new Font("",1,20));
				g.drawString(h.getRank()+"", Race.WIDTH-200-10, h.getY()+h.getH()/2);
			}
		}
